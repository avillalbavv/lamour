...
export function SiteMotion() {
  ...
  useEffect(() => {
    ...
    const collect = () => {
      main.querySelectorAll<HTMLElement>('.section-head,.category-tile,.product-card,.brand-story,.order-section>div,.privacy-grid>div,.newsletter>div,.newsletter>form,.page-heading,.detail-grid>div').forEach(element => {
    ...
    const stopMotion = () => {
      ...
      tracked.forEach(element => {element.dataset.reveal='visible';});
    };
    ...
    return () => {
      ...
      tracked.forEach(element => {delete element.dataset.reveal;});
    };
  }, [pathname]);
  return null;
}
